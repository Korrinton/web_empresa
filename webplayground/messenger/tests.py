...
class ThreadTestCase(TestCase):
    ...
    def test_add_message_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content='Hola usuario1')
        message2 = Message.objects.create(user=self.user2, content='Hola usuario2')
    ...
    def test_as_message_not_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content='Hola usuario1')
        message2 = Message.objects.create(user=self.user2, content='Hola usuario2')
        message3 = Message.objects.create(user=self.user3, content='Soy el espia')
